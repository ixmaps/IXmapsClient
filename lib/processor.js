    ...
    request.post(SUBMIT_URI, {
      ...
      function(err, res, body) {
        if (!err && res.statusCode == 200) {
          var trid = res.body.replace(/[\s\S]* ID=/m, '').replace(/[^\d][\s\S]*/m, '');
          send('submitted', `${options.dest} (${options.dest_ip}) ${trid} `);
        } else {
          send('submitted', `${options.deest} (${options.dest_ip}) error: ${err}`);
        }
