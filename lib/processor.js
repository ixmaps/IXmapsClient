    ...
    request.post(SUBMIT_URI, {
      ...
      function(err, res, body) {
        if (!err && res.statusCode == 200) {
          var trid = JSON.parse(res.body).TRid;
          send('submitted', `Processed ${options.dest} (${options.dest_ip}) - TRID ${trid}`, {options, trid});
        } else {
          send('submitted', `Error for ${options.dest} (${options.dest_ip}) ${err}`, {options, err});
        }
