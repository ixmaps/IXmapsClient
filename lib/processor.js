    ...
    trsets.getTrset(allOptions.trset, function(err, dests) {
      ...
      if (err) {
        ...
        var submitter = function(err, res) {
          ...
          if (dest && !cancelTrace) {
            send('STATUS', 'Host ' + dest + '  - ' + (total - dests.length) + ' / ' + total + ' in trset');
    ...
    request.post(SUBMIT_URI, {
      ...
      function(err, res, body) {
        if (!err && res.statusCode == 200) {
          var trid;
          try {
            trid = JSON.parse(res.body).TRid;
          } catch (e) {
            send('error', 'submission error ' + JSON.stringify(e));
            return;
          }
          send('submitted', `Processed ${options.dest} (${options.dest_ip}) - TRID ${trid}`, {trid});
        } else {
