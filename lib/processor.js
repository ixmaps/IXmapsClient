      ...
      if (err) {
        ...
        var submitter = function(err, res) {
          ...
          if (dest && !cancelTrace) {
            ...
            send('STATUS', 'TR Set completed. ' + succeeded + ' of ' + total + ' successfully contributed.', {status: 'done'});
          }
        };
        submitter();
      }
...
function doTrace(options, send, doneDestCallback) {
  ...
  if (!net.isIP(options.dest)) {
    ...
    dns.resolve4(options.dest, function(err, addresses) {
      if (err) {
        send('STATUS', 'done');
        doneDestCallback('cannot resolve host "' + options.dest + '"');
      } else {
      ...
      function(err, res, body) {
        if (!err && res.statusCode == 200) {
          ...
          try {
            ...
            send('error', 'submission error ' + JSON.stringify(e));
            return;
          }
          submitted++;
          send('submitted', `Processed ${options.dest} (${options.dest_ip}) - TRID ${trid}`, {trid, submissionMessage, dest: options.dest, dest_ip: options.dest_ip, submitted});
        } else {
          send('submitted', `Error for ${options.dest} (${options.dest_ip}) ${err}`, {err});
        }
